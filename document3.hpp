...
namespace krystal {
	...
	class document_builder : public reader_delegate {
		basic_value<CharT> root_, *cur_node_ = nullptr;
		std::vector<basic_value<CharT>*> context_stack_;
		std::string next_key_;
		
		friend class reader;
		
		void append(basic_value<CharT> val) {
			bool val_is_container = val.is_container();
			basic_value<CharT>* mv;
	...
	auto parse(ForwardIterator first, ForwardIterator last)
		-> basic_value<typename std::iterator_traits<ForwardIterator>::value_type>
	{
	...
	auto parse_stream(IStream &is) -> basic_value<char> {
		is >> std::noskipws;
	...
	template <typename CharT>
	basic_value<CharT> parse_string(std::basic_string<CharT> json_string) {
